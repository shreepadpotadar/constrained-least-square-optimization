...
{
    int num_elements = num_rows_ * num_rows_;
    ...
    Eigen::Map<Eigen::MatrixXd> matrix( random_nums, num_rows_, num_cols_ );
...
{
    int num_elements = num_rows_ * num_cols_;
    double* random_nums = new double[num_elements];
    ...
    for(int i = 0; i < num_elements; i++)
    {
        ...
        std::normal_distribution<double> d(0, 1/ sqrt(num_cols_)); 
